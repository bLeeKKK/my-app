...
export async function getAgingReport(data, sort) {
  ...
  return request(`${SERVER_PATH}/${BIZLOG_CORE}/exportTask/queryByPage`, {
    method: 'post',
    ...search,
  });
}
...
// 导出接口记录池数据
export async function exportAgingReport(data: ParamsType): Promise<{ data: unknown[] }> {
  return request(`${SERVER_PATH}/${BIZLOG_CORE}/fullLinkRecord/exportAgingReport`, {
    method: 'POST',
    responseType: 'blob',
    data,
  });
